"""Create the isolated helper venv used for CTC beam search + KenLM decoding."""
...
def _requirements_path() -> Path:
    return _phase1_dir() / "requirements-ctc-kenlm.txt"


def _default_venv_dir() -> Path:
    return _phase1_dir() / ".ctc-kenlm-venv"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Create the phase1 KenLM helper environment.")
...
def main() -> None:
    ...
    if version >= (3, 13):
        raise RuntimeError(
            "The CTC KenLM helper currently requires Python 3.11 or 3.12. "
            "The kenlm package does not build cleanly on Python 3.13 in this repo environment."
        )

    venv_dir = Path(args.venv_dir)
    requirements = Path(args.requirements)
    if not requirements.exists():
        raise FileNotFoundError(f"Missing helper requirements file: {requirements}")
    ...
    print(f"[CTC-KENLM] helper ready at {venv_dir / 'bin' / 'python'}")
